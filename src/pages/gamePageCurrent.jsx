...
import Headshot from '../components/RickyPearsall.png';
import Silhouette from '../components/RickyPearsallS.png';
...
const GamePage = () =>
{
    ...
    useEffect(() =>
    {
        const fetchGuesses = async () =>
        {
            ...
            const { data, error } = await supabase
                .from('users')
                .select('guesses_current, game_status_current, last_played_date_current')
                .eq('id', userId)
                .single();
            ...
            if (!error && data)
            {
                if (data.last_played_date_current !== today)
                {
                    await supabase
                        .from('users')
                        .update({
                            ...
                            last_played_date_current: today
                        })
                        .eq('id', userId);
                    ...
                    setIsCorrect(data.game_status_current === 'won');
                    setIsGameOver(data.game_status_current === 'lost');
                }
            };
        }
    ...
    const handleGuess = async (e) =>
    {
        ...
        if (guess.trim().toLowerCase() === player.name.toLowerCase()) {
            ...
            await supabase
                .from('users')
                .update({ wins_current: newWins, guesses_current: 4, game_status_current: 'won', last_played_date_current: today  })
                .eq('id', userId);
        } else {
            ...
            if (newGuesses <= 0) {
                ...
                await supabase
                    .from('users')
                    .update({ guesses_current: 4, game_status_current: 'lost', last_played_date_current: today })
                    .eq('id', userId);
            } else {
                await supabase
                    .from('users')
                    .update({ guesses_current: newGuesses, game_status_current: 'in_progress', last_played_date_current: today })
                    .eq('id', userId);
            }
        }
        setGuess('');
